...
> 엘라스틱 서치에 대해서 배웠다. 정말 많은 데이터들을 다루는 데이터베이스인데 진짜 무겁다는 것이 느껴진다. 하지만 무거운만큼 속도만큼은 정말 빠르다. 300만개의 데이터를 가지고 psql과 비교도 해보았다. 그런데 elasticsearch는 정말로 빨랐다. 많은 양의 데이터를 가지고 노는 것도 나름 재미있었다. 물론 300만개의 데이터를 넣는데만 이틀이나 걸렸지만 말이다.

> promise객체를 사용할 줄만 알았지 제대로 만들어 본 적은 없었다. 그래서 이번 기회에 control함수를 구현하여 직접 만들어봤다. 확실히 만들고나니까 코드가 깔끔해지는 것이 느껴진다. 